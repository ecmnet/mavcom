 ...
 *
 */
public class MAVLinkCRC_Obsolete {
	...
	/**
	 * Accumulate the X.25 CRC by adding one char at a time. The checksum function
	 * adds the hash of one char at a time to the 16 bit checksum
	 * 
	 * @param data new char to hash
	 * @param crc  the already accumulated checksum
	 * @return the new accumulated checksum
	 */
	private static int tmp, tmpdata, tmp4, crch, crcaccum, tmp8, tmp3, tmpa, tmpb;

	public static int crc_accumulate(byte data, int crc) {

		crcaccum = crc & 0x000000ff;
		...
		return crc;
		/*
		 * byte ch = (byte) (data ^ (byte) (crc & 0x00ff)); ch = (byte) (ch ^ (ch <<
		 * 4)); return ((crc >> 8) ^ (ch << 8) ^ (ch << 3) ^ (ch >> 4));
		 */
	}
	...
	/**
	 * Calculates the X.25 checksum to decode a MAVLink message stored in a byte
	 * buffer and begins after the MAVLink Packet start
	 * 
	...
	/**
	 * Calculates the X.25 checksum to encode a MAVLink message stored in a byte
	 * buffer and begins after the MAVLink Packet start
	 * 
	 * @param buffer     buffer containing the byte array to hash
	 * @param dataLength total length of the MAVLink message so CRC is calculate
	 *                   between 1 and length - 2
	 * @return the accumulated checksum
	 */
