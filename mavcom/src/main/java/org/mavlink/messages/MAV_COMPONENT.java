...
/**
 * Interface MAV_COMPONENT Component ids (values) for the different types and
 * instances of onboard hardware/software that might make up a MAVLink system
 * (autopilot, cameras, servos, GPS systems, avoidance systems etc.). Components
 * must use the appropriate ID in their source address when sending messages.
 * Components can also use IDs to determine if they are the intended recipient
 * of an incoming message. The MAV_COMP_ID_ALL value is used to indicate
 * messages that must be processed by all components. When creating new entries,
 * components that can have multiple instances (e.g. cameras, servos etc.)
 * should be allocated sequential values. An appropriate number of values should
 * be left free after these components to allow the number of instances to be
 * expanded.
 **/
public interface MAV_COMPONENT {
	 ...
	 * some generic functionalities, such as settings system parameters and
	 * monitoring the status of some processes that don't directly speak mavlink and
	 * so on.
	 */
