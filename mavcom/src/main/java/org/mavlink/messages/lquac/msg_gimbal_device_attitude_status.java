...
/**
 * Class msg_gimbal_device_attitude_status Message reporting the status of a
 * gimbal device. This message should be broadcasted by a gimbal device
 * component. The angles encoded in the quaternion are relative to absolute
 * North if the flag GIMBAL_DEVICE_FLAGS_YAW_LOCK is set (roll: positive is
 * rolling to the right, pitch: positive is pitching up, yaw is turn to the
 * right) or relative to the vehicle heading if the flag is not set. This
...
public class msg_gimbal_device_attitude_status extends MAVLinkMessage {
	...
	/**
	 * Quaternion components, w, x, y, z (1 0 0 0 is the null-rotation, the frame is
	 * depends on whether the flag GIMBAL_DEVICE_FLAGS_YAW_LOCK is set)
	 */
	...
	public String toString() {
		return "MAVLINK_MSG_ID_GIMBAL_DEVICE_ATTITUDE_STATUS : " + "  time_boot_ms=" + time_boot_ms + "  q[0]="
				+ format((float) q[0]) + "  q[1]=" + format((float) q[1]) + "  q[2]=" + format((float) q[2]) + "  q[3]="
				+ format((float) q[3]) + "  angular_velocity_x=" + format((float) angular_velocity_x)
				+ "  angular_velocity_y=" + format((float) angular_velocity_y) + "  angular_velocity_z="
				+ format((float) angular_velocity_z) + "  failure_flags=" + failure_flags + "  flags=" + flags
