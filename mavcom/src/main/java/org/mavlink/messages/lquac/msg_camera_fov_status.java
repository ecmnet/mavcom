...
/**
 * Class msg_camera_fov_status Information about the field of view of a camera.
 * Can be requested with a MAV_CMD_REQUEST_MESSAGE command.
 **/
public class msg_camera_fov_status extends MAVLinkMessage {
	...
	/**
	 * Latitude of center of image (INT32_MAX if unknown, INT32_MIN if at infinity,
	 * not intersecting with horizon).
	 */
	public long lat_image;
	/**
	 * Longitude of center of image (INT32_MAX if unknown, INT32_MIN if at infinity,
	 * not intersecting with horizon).
	 */
	...
	public String toString() {
		return "MAVLINK_MSG_ID_CAMERA_FOV_STATUS : " + "  time_boot_ms=" + time_boot_ms + "  lat_camera=" + lat_camera
				+ "  lon_camera=" + lon_camera + "  alt_camera=" + alt_camera + "  lat_image=" + lat_image
				+ "  lon_image=" + lon_image + "  alt_image=" + alt_image + "  q[0]=" + format((float) q[0]) + "  q[1]="
				+ format((float) q[1]) + "  q[2]=" + format((float) q[2]) + "  q[3]=" + format((float) q[3]) + "  hfov="
				+ format((float) hfov) + "  vfov=" + format((float) vfov);
	}
