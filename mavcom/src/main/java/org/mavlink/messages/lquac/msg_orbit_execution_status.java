...
/**
 * Class msg_orbit_execution_status Vehicle status report that is sent out while
 * orbit execution is in progress (see MAV_CMD_DO_ORBIT).
 **/
public class msg_orbit_execution_status extends MAVLinkMessage {
	...
	/**
	 * Radius of the orbit circle. Positive values orbit clockwise, negative values
	 * orbit counter-clockwise.
	 */
	public float radius;
	/**
	 * X coordinate of center point. Coordinate system depends on frame field: local
	 * = x position in meters * 1e4, global = latitude in degrees * 1e7.
	 */
	public long x;
	/**
	 * Y coordinate of center point. Coordinate system depends on frame field: local
	 * = x position in meters * 1e4, global = latitude in degrees * 1e7.
	 */
	public long y;
	/**
	...
	public String toString() {
		return "MAVLINK_MSG_ID_ORBIT_EXECUTION_STATUS : " + "  time_usec=" + time_usec + "  radius="
				+ format((float) radius) + "  x=" + x + "  y=" + y + "  z=" + format((float) z) + "  frame=" + frame;
	}
