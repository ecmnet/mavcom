...
/**
 * Class msg_safety_set_allowed_area Set a safety zone (volume), which is
 * defined by two corners of a cube. This message can be used to tell the MAV
 * which setpoints/waypoints to accept and which to reject. Safety areas are
...
public class msg_safety_set_allowed_area extends MAVLinkMessage {
	...
	public String toString() {
		return "MAVLINK_MSG_ID_SAFETY_SET_ALLOWED_AREA : " + "  p1x=" + format((float) p1x) + "  p1y="
				+ format((float) p1y) + "  p1z=" + format((float) p1z) + "  p2x=" + format((float) p2x) + "  p2y="
				+ format((float) p2y) + "  p2z=" + format((float) p2z) + "  target_system=" + target_system
