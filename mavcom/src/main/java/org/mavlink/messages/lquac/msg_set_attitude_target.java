...
/**
 * Class msg_set_attitude_target Sets a desired vehicle attitude. Used by an
 * external controller to command the vehicle (manual controller or other
 * system).
 **/
public class msg_set_attitude_target extends MAVLinkMessage {
	...
	public String toString() {
		return "MAVLINK_MSG_ID_SET_ATTITUDE_TARGET : " + "  time_boot_ms=" + time_boot_ms + "  q[0]="
				+ format((float) q[0]) + "  q[1]=" + format((float) q[1]) + "  q[2]=" + format((float) q[2]) + "  q[3]="
				+ format((float) q[3]) + "  body_roll_rate=" + format((float) body_roll_rate) + "  body_pitch_rate="
				+ format((float) body_pitch_rate) + "  body_yaw_rate=" + format((float) body_yaw_rate) + "  thrust="
				+ format((float) thrust) + "  target_system=" + target_system + "  target_component=" + target_component
				+ "  type_mask=" + type_mask + "  thrust_body[0]=" + format((float) thrust_body[0])
				+ "  thrust_body[1]=" + format((float) thrust_body[1]) + "  thrust_body[2]="
				+ format((float) thrust_body[2]);
	}
