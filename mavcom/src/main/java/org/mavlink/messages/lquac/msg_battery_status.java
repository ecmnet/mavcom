...
/**
 * Class msg_battery_status Battery information. Updates GCS with flight
 * controller battery status. Smart batteries also use this message, but may
...
public class msg_battery_status extends MAVLinkMessage {
	...
	/**
	 * Battery voltage of cells 1 to 10 (see voltages_ext for cells 11-14). Cells in
	 * this field above the valid cell count for this battery should have the
	 * UINT16_MAX value. If individual cell voltages are unknown or not measured for
	 * this battery, then the overall battery voltage should be filled in cell 0,
	 * with all others set to UINT16_MAX. If the voltage of the battery is greater
	 * than (UINT16_MAX - 1), then cell 0 should be set to (UINT16_MAX - 1), and
	 * cell 1 to the remaining voltage. This can be extended to multiple cells if
	 * the total voltage is greater than 2 * (UINT16_MAX - 1).
	 */
	...
	/**
	 * Battery mode. Default (0) is that battery mode reporting is not supported or
	 * battery is in normal-use mode.
	 */
	public int mode;
	...
	public String toString() {
		return "MAVLINK_MSG_ID_BATTERY_STATUS : " + "  current_consumed=" + current_consumed + "  energy_consumed="
				+ energy_consumed + "  temperature=" + temperature + "  voltages[0]=" + voltages[0] + "  voltages[1]="
				+ voltages[1] + "  voltages[2]=" + voltages[2] + "  voltages[3]=" + voltages[3] + "  voltages[4]="
				+ voltages[4] + "  voltages[5]=" + voltages[5] + "  voltages[6]=" + voltages[6] + "  voltages[7]="
				+ voltages[7] + "  voltages[8]=" + voltages[8] + "  voltages[9]=" + voltages[9] + "  current_battery="
				+ current_battery + "  id=" + id + "  battery_function=" + battery_function + "  type=" + type
				+ "  battery_remaining=" + battery_remaining + "  time_remaining=" + time_remaining + "  fault_bitmask="
				+ fault_bitmask + "  voltages_ext[0]=" + voltages_ext[0] + "  voltages_ext[1]=" + voltages_ext[1]
				+ "  voltages_ext[2]=" + voltages_ext[2] + "  voltages_ext[3]=" + voltages_ext[3] + "  charge_state="
				+ charge_state + "  mode=" + mode;
	}
