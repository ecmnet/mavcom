...
/**
 * Class msg_winch_status Winch status.
 **/
public class msg_winch_status extends MAVLinkMessage {
	...
	/**
	 * Speed line is being released or retracted. Positive values if being released,
	...
	public String toString() {
		return "MAVLINK_MSG_ID_WINCH_STATUS : " + "  time_usec=" + time_usec + "  line_length="
				+ format((float) line_length) + "  speed=" + format((float) speed) + "  tension="
				+ format((float) tension) + "  voltage=" + format((float) voltage) + "  current="
				+ format((float) current) + "  status=" + status + "  temperature=" + temperature;
	}
