...
/**
 * Class msg_local_position_ned_system_global_offset The offset in X, Y, Z and
 * yaw between the LOCAL_POSITION_NED messages of MAV X and the global
 * coordinate frame in NED coordinates. Coordinate frame is right-handed, Z-axis
...
public class msg_local_position_ned_system_global_offset extends MAVLinkMessage {
	...
	public String toString() {
		return "MAVLINK_MSG_ID_LOCAL_POSITION_NED_SYSTEM_GLOBAL_OFFSET : " + "  time_boot_ms=" + time_boot_ms + "  x="
				+ format((float) x) + "  y=" + format((float) y) + "  z=" + format((float) z) + "  roll="
				+ format((float) roll) + "  pitch=" + format((float) pitch) + "  yaw=" + format((float) yaw);
	}
