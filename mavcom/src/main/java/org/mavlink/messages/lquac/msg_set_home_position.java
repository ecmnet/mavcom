...
/**
 * Class msg_set_home_position The position the system will return to and land
 * on. The position is set automatically by the system during the takeoff in
 * case it was not explicitly set by the operator before or after. The global
...
public class msg_set_home_position extends MAVLinkMessage {
	...
	public String toString() {
		return "MAVLINK_MSG_ID_SET_HOME_POSITION : " + "  latitude=" + latitude + "  longitude=" + longitude
				+ "  altitude=" + altitude + "  x=" + format((float) x) + "  y=" + format((float) y) + "  z="
				+ format((float) z) + "  q[0]=" + format((float) q[0]) + "  q[1]=" + format((float) q[1]) + "  q[2]="
				+ format((float) q[2]) + "  q[3]=" + format((float) q[3]) + "  approach_x=" + format((float) approach_x)
				+ "  approach_y=" + format((float) approach_y) + "  approach_z=" + format((float) approach_z)
