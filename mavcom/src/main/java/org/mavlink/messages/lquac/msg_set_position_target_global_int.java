...
/**
 * Class msg_set_position_target_global_int Sets a desired vehicle position,
 * velocity, and/or acceleration in a global coordinate system (WGS84). Used by
 * an external controller to command the vehicle (manual controller or other
 * system).
 **/
public class msg_set_position_target_global_int extends MAVLinkMessage {
	...
	public String toString() {
		return "MAVLINK_MSG_ID_SET_POSITION_TARGET_GLOBAL_INT : " + "  time_boot_ms=" + time_boot_ms + "  lat_int="
				+ lat_int + "  lon_int=" + lon_int + "  alt=" + format((float) alt) + "  vx=" + format((float) vx)
				+ "  vy=" + format((float) vy) + "  vz=" + format((float) vz) + "  afx=" + format((float) afx)
				+ "  afy=" + format((float) afy) + "  afz=" + format((float) afz) + "  yaw=" + format((float) yaw)
				+ "  yaw_rate=" + format((float) yaw_rate) + "  type_mask=" + type_mask + "  target_system="
				+ target_system + "  target_component=" + target_component + "  coordinate_frame=" + coordinate_frame;
	}
