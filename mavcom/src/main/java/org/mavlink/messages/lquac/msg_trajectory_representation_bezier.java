...
/**
 * Class msg_trajectory_representation_bezier Describe a trajectory using an
 * array of up-to 5 bezier control points in the local frame
 * (MAV_FRAME_LOCAL_NED).
 **/
public class msg_trajectory_representation_bezier extends MAVLinkMessage {
	...
	public String toString() {
		return "MAVLINK_MSG_ID_TRAJECTORY_REPRESENTATION_BEZIER : " + "  time_usec=" + time_usec + "  pos_x[0]="
				...
				+ format((float) pos_z[4]) + "  delta[0]=" + format((float) delta[0]) + "  delta[1]="
				+ format((float) delta[1]) + "  delta[2]=" + format((float) delta[2]) + "  delta[3]="
				+ format((float) delta[3]) + "  delta[4]=" + format((float) delta[4]) + "  pos_yaw[0]="
				+ format((float) pos_yaw[0]) + "  pos_yaw[1]=" + format((float) pos_yaw[1]) + "  pos_yaw[2]="
				+ format((float) pos_yaw[2]) + "  pos_yaw[3]=" + format((float) pos_yaw[3]) + "  pos_yaw[4]="
				+ format((float) pos_yaw[4]) + "  valid_points=" + valid_points;
	}
