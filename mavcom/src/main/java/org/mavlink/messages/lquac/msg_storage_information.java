...
/**
 * Class msg_storage_information Information about a storage medium. This
 * message is sent in response to a request with MAV_CMD_REQUEST_MESSAGE and
 * whenever the status of the storage changes (STORAGE_STATUS). Use
 * MAV_CMD_REQUEST_MESSAGE.param2 to indicate the index/id of requested storage:
 * 0 for all, 1 for first, 2 for second, etc.
 **/
public class msg_storage_information extends MAVLinkMessage {
	...
	/**
	 * Total capacity. If storage is not ready (STORAGE_STATUS_READY) value will be
	 * ignored.
	 */
	...
	public int type;
	/**
	 * Textual storage name to be used in UI (microSD 1, Internal Memory, etc.) This
	 * is a NULL terminated string. If it is exactly 32 characters long, add a
	 * terminating NULL. If this string is empty, the generic type is shown to the
	 * user.
	 */
	 ...
	 * system-default storage id used for saving media (if possible/supported). This
	 * setting can then be overridden using MAV_CMD_SET_STORAGE_USAGE. If the media
	 * usage flags are not set, a GCS may assume storage ID 1 is the default storage
	 * for all media types.
	 */
	...
	public String toString() {
		return "MAVLINK_MSG_ID_STORAGE_INFORMATION : " + "  time_boot_ms=" + time_boot_ms + "  total_capacity="
				+ format((float) total_capacity) + "  used_capacity=" + format((float) used_capacity)
				+ "  available_capacity=" + format((float) available_capacity) + "  read_speed="
				+ format((float) read_speed) + "  write_speed=" + format((float) write_speed) + "  storage_id="
				+ storage_id + "  storage_count=" + storage_count + "  status=" + status + "  type=" + type + "  name="
				+ getName() + "  storage_usage=" + storage_usage;
	}
