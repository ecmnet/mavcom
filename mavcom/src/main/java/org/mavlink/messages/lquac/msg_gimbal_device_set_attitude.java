...
/**
 * Class msg_gimbal_device_set_attitude Low level message to control a gimbal
 * device's attitude. This message is to be sent from the gimbal manager to the
 * gimbal device component. Angles and rates can be set to NaN according to use
 * case.
 **/
public class msg_gimbal_device_set_attitude extends MAVLinkMessage {
	 ...
	 * depends on whether the flag GIMBAL_DEVICE_FLAGS_YAW_LOCK is set, set all
	 * fields to NaN if only angular velocity should be used)
	 */
	public float[] q = new float[4];
	/**
	 * X component of angular velocity, positive is rolling to the right, NaN to be
	 * ignored.
	 */
	...
	public String toString() {
		return "MAVLINK_MSG_ID_GIMBAL_DEVICE_SET_ATTITUDE : " + "  q[0]=" + format((float) q[0]) + "  q[1]="
				+ format((float) q[1]) + "  q[2]=" + format((float) q[2]) + "  q[3]=" + format((float) q[3])
				+ "  angular_velocity_x=" + format((float) angular_velocity_x) + "  angular_velocity_y="
				+ format((float) angular_velocity_y) + "  angular_velocity_z=" + format((float) angular_velocity_z)
				+ "  flags=" + flags + "  target_system=" + target_system + "  target_component=" + target_component;
	}
