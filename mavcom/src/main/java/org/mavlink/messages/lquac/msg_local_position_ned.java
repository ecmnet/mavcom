...
/**
 * Class msg_local_position_ned The filtered local position (e.g. fused computer
 * vision and accelerometers). Coordinate frame is right-handed, Z-axis down
...
public class msg_local_position_ned extends MAVLinkMessage {
	...
	public String toString() {
		return "MAVLINK_MSG_ID_LOCAL_POSITION_NED : " + "  time_boot_ms=" + time_boot_ms + "  x=" + format((float) x)
				+ "  y=" + format((float) y) + "  z=" + format((float) z) + "  vx=" + format((float) vx) + "  vy="
				+ format((float) vy) + "  vz=" + format((float) vz);
	}
