...
/**
 * Class msg_mission_item Message encoding a mission item. This message is
 * emitted to announce the presence of a mission item and to set a mission item
 * on the system. The mission item can be either in x, y, z meters (type: LOCAL)
 * or x:lat, y:lon, z:altitude. Local frame is Z-down, right handed (NED),
 * global frame is Z-up, right handed (ENU). NaN may be used to indicate an
...
public class msg_mission_item extends MAVLinkMessage {
	...
	public String toString() {
		return "MAVLINK_MSG_ID_MISSION_ITEM : " + "  param1=" + format((float) param1) + "  param2="
				+ format((float) param2) + "  param3=" + format((float) param3) + "  param4=" + format((float) param4)
				+ "  x=" + format((float) x) + "  y=" + format((float) y) + "  z=" + format((float) z) + "  seq=" + seq
				+ "  command=" + command + "  target_system=" + target_system + "  target_component=" + target_component
				+ "  frame=" + frame + "  current=" + current + "  autocontinue=" + autocontinue + "  mission_type="
				+ mission_type;
	}
