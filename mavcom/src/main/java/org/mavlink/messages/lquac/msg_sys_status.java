...
/**
 * Class msg_sys_status The general system state. If the system is following the
 * MAVLink standard, the system state is mainly defined by three orthogonal
 * states/modes: The system mode, which is either LOCKED (motors shut down and
 * locked), MANUAL (system under RC control), GUIDED (system with autonomous
 * position control, position setpoint controlled manually) or AUTO (system
 * guided by path/waypoint planner). The NAV_MODE defined the current flight
 * state: LIFTOFF (often an open-loop maneuver), LANDING, WAYPOINTS or VECTOR.
 * This represents the internal navigation state machine. The system status
 * shows whether the system is currently active or not and if an emergency
 * occurred. During the CRITICAL and EMERGENCY states the MAV is still
 * considered to be active, but should start emergency procedures autonomously.
 * After a failure occurred it should first move from active to critical to
 * allow manual intervention and then move to emergency after a certain timeout.
 **/
public class msg_sys_status extends MAVLinkMessage {
	...
	/**
	 * Bitmap showing which onboard controllers and sensors are present. Value of 0:
	 * not present. Value of 1: present.
	 */
	public long onboard_control_sensors_present;
	/**
	 * Bitmap showing which onboard controllers and sensors are enabled: Value of 0:
	...
	public void decode(LittleEndianDataInputStream dis) throws IOException {
		...
		onboard_control_sensors_present_extended = (int) dis.readInt() & 0x00FFFFFFFF;
		onboard_control_sensors_enabled_extended = (int) dis.readInt() & 0x00FFFFFFFF;
		onboard_control_sensors_health_extended = (int) dis.readInt() & 0x00FFFFFFFF;
	}
	...
	public byte[] encode() throws IOException {
		...
		dos.writeInt((int) (onboard_control_sensors_present_extended & 0x00FFFFFFFF));
		dos.writeInt((int) (onboard_control_sensors_enabled_extended & 0x00FFFFFFFF));
		dos.writeInt((int) (onboard_control_sensors_health_extended & 0x00FFFFFFFF));
		dos.flush();
	...
	public String toString() {
		return "MAVLINK_MSG_ID_SYS_STATUS : " + "  onboard_control_sensors_present=" + onboard_control_sensors_present
				...
				+ "  onboard_control_sensors_health=" + onboard_control_sensors_health + "  load=" + load
				+ "  voltage_battery=" + voltage_battery + "  current_battery=" + current_battery + "  drop_rate_comm="
				+ drop_rate_comm + "  errors_comm=" + errors_comm + "  errors_count1=" + errors_count1
				+ "  errors_count2=" + errors_count2 + "  errors_count3=" + errors_count3 + "  errors_count4="
				+ errors_count4 + "  battery_remaining=" + battery_remaining
				+ "  onboard_control_sensors_present_extended=" + onboard_control_sensors_present_extended
				+ "  onboard_control_sensors_enabled_extended=" + onboard_control_sensors_enabled_extended
				+ "  onboard_control_sensors_health_extended=" + onboard_control_sensors_health_extended;
	}
