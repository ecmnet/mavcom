...
/**
 * Class msg_gimbal_device_information Information about a low level gimbal.
 * This message should be requested by the gimbal manager or a ground station
 * using MAV_CMD_REQUEST_MESSAGE. The maximum angles and rates are the limits by
 * hardware. However, the limits by software used are likely different/smaller
 * and dependent on mode/settings/etc..
 **/
public class msg_gimbal_device_information extends MAVLinkMessage {
	...
	public String toString() {
		return "MAVLINK_MSG_ID_GIMBAL_DEVICE_INFORMATION : " + "  uid=" + uid + "  time_boot_ms=" + time_boot_ms
				+ "  firmware_version=" + firmware_version + "  hardware_version=" + hardware_version + "  roll_min="
				+ format((float) roll_min) + "  roll_max=" + format((float) roll_max) + "  pitch_min="
				+ format((float) pitch_min) + "  pitch_max=" + format((float) pitch_max) + "  yaw_min="
				+ format((float) yaw_min) + "  yaw_max=" + format((float) yaw_max) + "  cap_flags=" + cap_flags
				+ "  custom_cap_flags=" + custom_cap_flags + "  vendor_name=" + getVendor_name() + "  model_name="
				+ getModel_name() + "  custom_name=" + getCustom_name();
	}
