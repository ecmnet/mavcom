...
/**
 * Class msg_esc_status ESC information for higher rate streaming. Recommended
 * streaming rate is ~10 Hz. Information that changes more slowly is sent in
 * ESC_INFO. It should typically only be streamed on high-bandwidth links (i.e.
 * to a companion computer).
 **/
public class msg_esc_status extends MAVLinkMessage {
	...
	public String toString() {
		return "MAVLINK_MSG_ID_ESC_STATUS : " + "  time_usec=" + time_usec + "  rpm[0]=" + rpm[0] + "  rpm[1]=" + rpm[1]
				+ "  rpm[2]=" + rpm[2] + "  rpm[3]=" + rpm[3] + "  voltage[0]=" + format((float) voltage[0])
				+ "  voltage[1]=" + format((float) voltage[1]) + "  voltage[2]=" + format((float) voltage[2])
				+ "  voltage[3]=" + format((float) voltage[3]) + "  current[0]=" + format((float) current[0])
				+ "  current[1]=" + format((float) current[1]) + "  current[2]=" + format((float) current[2])
				+ "  current[3]=" + format((float) current[3]) + "  index=" + index;
	}
