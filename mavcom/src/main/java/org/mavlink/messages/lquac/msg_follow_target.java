...
/**
 * Class msg_follow_target Current motion information from a designated system
 **/
public class msg_follow_target extends MAVLinkMessage {
	...
	public String toString() {
		return "MAVLINK_MSG_ID_FOLLOW_TARGET : " + "  timestamp=" + timestamp + "  custom_state=" + custom_state
				+ "  lat=" + lat + "  lon=" + lon + "  alt=" + format((float) alt) + "  vel[0]="
				+ format((float) vel[0]) + "  vel[1]=" + format((float) vel[1]) + "  vel[2]=" + format((float) vel[2])
				+ "  acc[0]=" + format((float) acc[0]) + "  acc[1]=" + format((float) acc[1]) + "  acc[2]="
				+ format((float) acc[2]) + "  attitude_q[0]=" + format((float) attitude_q[0]) + "  attitude_q[1]="
				+ format((float) attitude_q[1]) + "  attitude_q[2]=" + format((float) attitude_q[2])
				+ "  attitude_q[3]=" + format((float) attitude_q[3]) + "  rates[0]=" + format((float) rates[0])
				+ "  rates[1]=" + format((float) rates[1]) + "  rates[2]=" + format((float) rates[2])
				+ "  position_cov[0]=" + format((float) position_cov[0]) + "  position_cov[1]="
				+ format((float) position_cov[1]) + "  position_cov[2]=" + format((float) position_cov[2])
