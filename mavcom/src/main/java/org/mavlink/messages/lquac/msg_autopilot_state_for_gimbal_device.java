...
/**
 * Class msg_autopilot_state_for_gimbal_device Low level message containing
 * autopilot state relevant for a gimbal device. This message is to be sent from
 * the gimbal manager to the gimbal device component. The data of this message
 * server for the gimbal's estimator corrections in particular horizon
 * compensation, as well as the autopilot's control intention e.g. feed forward
...
public class msg_autopilot_state_for_gimbal_device extends MAVLinkMessage {
	...
	public String toString() {
		return "MAVLINK_MSG_ID_AUTOPILOT_STATE_FOR_GIMBAL_DEVICE : " + "  time_boot_us=" + time_boot_us + "  q[0]="
				+ format((float) q[0]) + "  q[1]=" + format((float) q[1]) + "  q[2]=" + format((float) q[2]) + "  q[3]="
				+ format((float) q[3]) + "  q_estimated_delay_us=" + q_estimated_delay_us + "  vx=" + format((float) vx)
				+ "  vy=" + format((float) vy) + "  vz=" + format((float) vz) + "  v_estimated_delay_us="
				+ v_estimated_delay_us + "  feed_forward_angular_velocity_z="
				+ format((float) feed_forward_angular_velocity_z) + "  estimator_status=" + estimator_status
				+ "  target_system=" + target_system + "  target_component=" + target_component + "  landed_state="
				+ landed_state;
	}
