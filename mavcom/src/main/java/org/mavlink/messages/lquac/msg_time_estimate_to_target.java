...
/**
 * Class msg_time_estimate_to_target Time/duration estimates for various events
 * and actions given the current vehicle state and position.
 **/
public class msg_time_estimate_to_target extends MAVLinkMessage {
	...
	/**
	 * Estimated time to complete the vehicle's configured "safe return" action from
	 * its current position (e.g. RTL, Smart RTL, etc.). -1 indicates that the
	...
	public long land;
	/**
	 * Estimated time for reaching/completing the currently active mission item. -1
	 * means no time estimate available.
	 */
	public long mission_next_item;
	/**
	 * Estimated time for completing the current mission. -1 means no mission active
	...
	public String toString() {
		return "MAVLINK_MSG_ID_TIME_ESTIMATE_TO_TARGET : " + "  safe_return=" + safe_return + "  land=" + land
				+ "  mission_next_item=" + mission_next_item + "  mission_end=" + mission_end + "  commanded_action="
				+ commanded_action;
	}
