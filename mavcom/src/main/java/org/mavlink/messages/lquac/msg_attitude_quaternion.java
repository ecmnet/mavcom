...
/**
 * Class msg_attitude_quaternion The attitude in the aeronautical frame
 * (right-handed, Z-down, X-front, Y-right), expressed as quaternion. Quaternion
 * order is w, x, y, z and a zero rotation would be expressed as (1 0 0 0).
 **/
public class msg_attitude_quaternion extends MAVLinkMessage {
	 ...
	 * This field is intended for systems in which the reference attitude may change
	 * during flight. For example, tailsitters VTOLs rotate their reference attitude
	 * by 90 degrees between hover mode and fixed wing mode, thus repr_offset_q is
	 * equal to [1, 0, 0, 0] in hover mode and equal to [0.7071, 0, 0.7071, 0] in
	 * fixed wing mode.
	 */
	...
	public String toString() {
		return "MAVLINK_MSG_ID_ATTITUDE_QUATERNION : " + "  time_boot_ms=" + time_boot_ms + "  q1=" + format((float) q1)
				+ "  q2=" + format((float) q2) + "  q3=" + format((float) q3) + "  q4=" + format((float) q4)
				+ "  rollspeed=" + format((float) rollspeed) + "  pitchspeed=" + format((float) pitchspeed)
				+ "  yawspeed=" + format((float) yawspeed) + "  repr_offset_q[0]=" + format((float) repr_offset_q[0])
				+ "  repr_offset_q[1]=" + format((float) repr_offset_q[1]) + "  repr_offset_q[2]="
				+ format((float) repr_offset_q[2]) + "  repr_offset_q[3]=" + format((float) repr_offset_q[3]);
	}
