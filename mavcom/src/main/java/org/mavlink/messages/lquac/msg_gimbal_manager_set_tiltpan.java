...
/**
 * Class msg_gimbal_manager_set_tiltpan High level message to control a gimbal's
 * tilt and pan angles. This message is to be sent to the gimbal manager (e.g.
...
public class msg_gimbal_manager_set_tiltpan extends MAVLinkMessage {
	...
	public float tilt;
	/**
	 * Pan/yaw angle (positive: to the right, negative: to the left, NaN to be
	 * ignored).
	 */
	public float pan;
	/**
	...
	/**
	 * Pan/yaw angular rate (positive: to the right, negative: to the left, NaN to
	 * be ignored).
	 */
	...
	public String toString() {
		return "MAVLINK_MSG_ID_GIMBAL_MANAGER_SET_TILTPAN : " + "  flags=" + flags + "  tilt=" + format((float) tilt)
				+ "  pan=" + format((float) pan) + "  tilt_rate=" + format((float) tilt_rate) + "  pan_rate="
				+ format((float) pan_rate) + "  target_system=" + target_system + "  target_component="
				+ target_component + "  gimbal_device_id=" + gimbal_device_id;
	}
}
