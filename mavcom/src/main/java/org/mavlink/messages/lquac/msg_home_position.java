...
/**
 * Class msg_home_position This message can be requested by sending the
 * MAV_CMD_GET_HOME_POSITION command. The position the system will return to and
 * land on. The position is set automatically by the system during the takeoff
 * in case it was not explicitly set by the operator before or after. The global
 * and local positions encode the position in the respective coordinate frames,
 * while the q parameter encodes the orientation of the surface. Under normal
 * conditions it describes the heading and terrain slope, which can be used by
 * the aircraft to adjust the approach. The approach 3D vector describes the
 * point to which the system should fly in normal flight mode and then perform a
...
public class msg_home_position extends MAVLinkMessage {
	 ...
	 * should set it the same way as multicopters. Runway-landing fixed wing
	 * aircraft should set it to the opposite direction of the takeoff, assuming the
	...
	public String toString() {
		return "MAVLINK_MSG_ID_HOME_POSITION : " + "  latitude=" + latitude + "  longitude=" + longitude + "  altitude="
				+ altitude + "  x=" + format((float) x) + "  y=" + format((float) y) + "  z=" + format((float) z)
				+ "  q[0]=" + format((float) q[0]) + "  q[1]=" + format((float) q[1]) + "  q[2]=" + format((float) q[2])
				+ "  q[3]=" + format((float) q[3]) + "  approach_x=" + format((float) approach_x) + "  approach_y="
				+ format((float) approach_y) + "  approach_z=" + format((float) approach_z) + "  time_usec="
				+ time_usec;
	}
