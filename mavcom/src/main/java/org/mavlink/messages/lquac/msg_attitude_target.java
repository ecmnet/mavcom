...
/**
 * Class msg_attitude_target Reports the current commanded attitude of the
 * vehicle as specified by the autopilot. This should match the commands sent in
 * a SET_ATTITUDE_TARGET message if the vehicle is being controlled this way.
 **/
public class msg_attitude_target extends MAVLinkMessage {
	...
	public String toString() {
		return "MAVLINK_MSG_ID_ATTITUDE_TARGET : " + "  time_boot_ms=" + time_boot_ms + "  q[0]=" + format((float) q[0])
				+ "  q[1]=" + format((float) q[1]) + "  q[2]=" + format((float) q[2]) + "  q[3]=" + format((float) q[3])
				+ "  body_roll_rate=" + format((float) body_roll_rate) + "  body_pitch_rate="
				+ format((float) body_pitch_rate) + "  body_yaw_rate=" + format((float) body_yaw_rate) + "  thrust="
				+ format((float) thrust) + "  type_mask=" + type_mask;
	}
