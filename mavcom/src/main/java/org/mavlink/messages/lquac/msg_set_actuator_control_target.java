...
/**
 * Class msg_set_actuator_control_target Set the vehicle attitude and body
 * angular rates.
 **/
public class msg_set_actuator_control_target extends MAVLinkMessage {
	 ...
	 * single rotation direction motors is 0..1, negative range for reverse
	 * direction. Standard mapping for attitude controls (group 0): (index 0-7):
	 * roll, pitch, yaw, throttle, flaps, spoilers, airbrakes, landing gear. Load a
	 * pass-through mixer to repurpose them as generic outputs.
	 */
	...
	public String toString() {
		return "MAVLINK_MSG_ID_SET_ACTUATOR_CONTROL_TARGET : " + "  time_usec=" + time_usec + "  controls[0]="
				+ format((float) controls[0]) + "  controls[1]=" + format((float) controls[1]) + "  controls[2]="
				+ format((float) controls[2]) + "  controls[3]=" + format((float) controls[3]) + "  controls[4]="
				+ format((float) controls[4]) + "  controls[5]=" + format((float) controls[5]) + "  controls[6]="
				+ format((float) controls[6]) + "  controls[7]=" + format((float) controls[7]) + "  group_mlx="
				+ group_mlx + "  target_system=" + target_system + "  target_component=" + target_component;
	}
