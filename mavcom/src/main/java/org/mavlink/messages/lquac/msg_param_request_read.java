...
/**
 * Class msg_param_request_read Request to read the onboard parameter with the
 * param_id string id. Onboard parameters are stored as key[const char*] ->
 * value[float]. This allows to send a parameter to any other component (such as
 * the GCS) without the need of previous knowledge of possible parameter names.
 * Thus the same GCS can store different parameters for different autopilots.
 * See also https://mavlink.io/en/services/parameter.html for a full
...
public class msg_param_request_read extends MAVLinkMessage {
	...
	public String toString() {
		return "MAVLINK_MSG_ID_PARAM_REQUEST_READ : " + "  param_index=" + param_index + "  target_system="
				+ target_system + "  target_component=" + target_component + "  param_id=" + getParam_id();
	}
