...
/**
 * Class msg_altitude The current system altitude.
 **/
public class msg_altitude extends MAVLinkMessage {
	 ...
	 * never reset, but represents the local altitude change). The only guarantee on
	 * this field is that it will never be reset and is consistent within a flight.
	 * The recommended value for this field is the uncorrected barometric altitude
	 * at boot time. This altitude will also drift and vary between flights.
	 */
	public float altitude_monotonic;
	/**
	 * This altitude measure is strictly above mean sea level and might be
	 * non-monotonic (it might reset on events like GPS lock or when a new QNH value
	 * is set). It should be the altitude to which global altitude waypoints are
	 * compared to. Note that it is *not* the GPS altitude, however, most GPS
	 ...
	 * altitude above home, but in reference to the coordinate origin (0, 0, 0). It
	 * is up-positive.
	 */
	...
	public String toString() {
		return "MAVLINK_MSG_ID_ALTITUDE : " + "  time_usec=" + time_usec + "  altitude_monotonic="
				+ format((float) altitude_monotonic) + "  altitude_amsl=" + format((float) altitude_amsl)
				+ "  altitude_local=" + format((float) altitude_local) + "  altitude_relative="
				+ format((float) altitude_relative) + "  altitude_terrain=" + format((float) altitude_terrain)
