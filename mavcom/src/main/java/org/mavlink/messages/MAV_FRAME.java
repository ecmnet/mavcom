...
/**
 * Interface MAV_FRAME Co-ordinate frames used by MAVLink. Not all frames are
 * supported by all commands, messages, or vehicles.
 * 
 * Global frames use the following naming conventions: - "GLOBAL": Global
 * co-ordinate frame with WGS84 latitude/longitude and altitude positive over
 * mean sea level (MSL) by default. The following modifiers may be used with
 * "GLOBAL": - "RELATIVE_ALT": Altitude is relative to the vehicle home position
 * rather than MSL. - "TERRAIN_ALT": Altitude is relative to ground level rather
 * than MSL. - "INT": Latitude/longitude (in degrees) are scaled by multiplying
 * by 1E7.
 * 
 * Local frames use the following naming conventions: - "LOCAL": Origin of local
 * frame is fixed relative to earth. Unless otherwise specified this origin is
 * the origin of the vehicle position-estimator ("EKF"). - "BODY": Origin of
 * local frame travels with the vehicle. NOTE, "BODY" does NOT indicate
 * alignment of frame axis with vehicle attitude. - "OFFSET": Deprecated synonym
 * for "BODY" (origin travels with the vehicle). Not to be used for new frames.
 * 
...
public interface MAV_FRAME {
	...
	/**
	 * ENU local tangent frame (x: East, y: North, z: Up) with origin fixed relative
	 * to earth.
	 */
	public final static int MAV_FRAME_LOCAL_ENU = 4;
	/**
	 * Global (WGS84) coordinate frame (scaled) + MSL altitude. First value / x:
	 * latitude in degrees*1E7, second value / y: longitude in degrees*1E7, third
	 ...
	 * position. First value / x: latitude in degrees*1E7, second value / y:
	 * longitude in degrees*1E7, third value / z: positive altitude with 0 being at
	 * the altitude of the home location.
	 */
	public final static int MAV_FRAME_GLOBAL_RELATIVE_ALT_INT = 6;
	/**
	 * NED local tangent frame (x: North, y: East, z: Down) with origin that travels
	 * with the vehicle.
	 */
	 ...
	 * longitude in degrees*1E7, third value / z: positive altitude in meters with 0
	 * being at ground level in terrain model.
	 */
	 ...
	 * travels with vehicle. The forward axis is aligned to the front of the vehicle
